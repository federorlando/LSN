...
double delta_best;
double delta_innesco = delta;
int N_K = 10;

ofstream DeltaRun;
DeltaRun.open("delta_gas.txt", ios::app);


for(int iblk=1; iblk <= 1000; ++iblk)
{
	...
	reject_rate = n_reject/(attempted*N_K);

	//cout << "Acceptance rate " << accepted/attempted << endl;
	//cout << "Rejection rate " << n_reject/attempted << endl;
...
DeltaRun << delta_innesco << "		" << delta_best << "		" << diff << endl;
